...
const randomStudents = students.map(student => student)						// An undestructive copy of the array (used for the shuffling)
const studentNames = randomStudents.map(student=> student.name);			// An array of the student's names (used for filtering)
...
startNewGame.addEventListener('click', e => {
	...
	startNewGame.classList.add('hide')			// Hiding button 'Start new game'
	gameModeEl.classList.remove('hide');		// Showing button options 10, 20 and All
	turnoutBox.classList.add('hide');			// Hiding the turnout box for next game
 });
